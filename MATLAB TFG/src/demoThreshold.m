function ITDt = demoThreshold(Obj)
...
%ITDt -> ITD vector with all angles on horizontal plane
    ...
    for x = 1:length(Phor)
        ITDt(x) = Threshold(Obj, Obj.SourcePosition(Phor(x),1), Obj.SourcePosition(Phor(x),2));
    ...
    figure();
    %stem(Obj.SourcePosition(Phor), ITDt, 'k'); title([Obj.GLOBAL_ListenerShortName ' ' Obj.GLOBAL_DatabaseName ' Threshold']);
    %xlabel('Azimuth (º)'); ylabel('ITDt (s)'); grid on; hold on;
    plot(Obj.SourcePosition(Phor,1), ITDw, 'g');
